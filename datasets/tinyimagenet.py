...
url = "https://github.com/tjmoon0104/pytorch-tiny-imagenet/releases/download/tiny-imagenet-dataset/tiny-imagenet-200.zip"
data_transforms = {
...
def load(root: str = "../data", **kwargs) -> Tuple[ImageFolder, ImageFolder]:
    print(f"""loading tinyimagenet...""")
    data_path = os.path.join(root, "TinyImageNet")

    trainset = TinyImageNet(data_path, 'train', data_transforms['train'], resolution=64)
    testset = TinyImageNet(data_path, 'test', data_transforms['test'], resolution=64)
    ...
    def load_data(self):
        ...
        if not self.zip_file.exists():
            ...
            with tqdm(unit="B", unit_scale=True, unit_divisor=1024, miniters=1, desc=url.split("/")[-1]) as t:
                ...
                urlretrieve(url=url, filename=self.zip_file, reporthook=show_progress)
